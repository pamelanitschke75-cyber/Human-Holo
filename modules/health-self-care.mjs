...
export const PAM_HOLO_PRIVATE_SELF_CARE_TEST_ACTIVE = true;

export function isHealthSelfCareRequest(
  message,
  { privatePamMedical = false } = {}
) {
  if (!privatePamMedical) return false;
...
Die medizinische Selbsthilfe und individuelle Gesundheitsberatung ist im
allgemeinen Human Holo deaktiviert. Erinnerungs- und Organisationsfunktionen
...
`;
  ...
  return `
PRIVATE PAM-HOLO-TESTFUNKTION · VORSICHTIGE GESUNDHEITSBEGLEITUNG:

Pam’s Holo darf bei leichten menschlichen Beschwerden und kleinen,
oberflächlichen Verletzungen konkrete, vorsichtige Schritte für zu Hause
erklären. Stelle aber niemals eine Diagnose und behaupte niemals, eine
Situation sei garantiert harmlos oder sicher.
...
Nenne gleichzeitig nur Zwischenmaßnahmen mit niedrigem Risiko.
...
Diese Gesundheitsbegleitung ist allgemeine unterstützende Information in Pams
privatem Test und ersetzt keine individuelle Untersuchung oder ärztliche
Beratung. Sage das kurz nach der praktischen Antwort.
`;
}
