...
(() => {
  ...
  if (
    ...
    console.error("Human Holo UI konnte nicht vorbereitet werden.");
    return;
  }
  ...
  async function loadGoogleStatus() {
    ...
    try {
      const response = await fetch(
        "https://pam-holo-edge-guard.pamela-nitschke75.workers.dev/calendar/status",
        { cache: "no-store" }
      );
  ...
  async function toggleWhatsAppDrivingMode() {
    ...
    try {
      ...
      if (!currentStatus.permissionGranted) {
        ...
        showToast(
          "Bitte erlaube Human Holo jetzt den Benachrichtigungszugriff. " +
  ...
  document.getElementById("googleAccountRow").addEventListener("click", () => {
    if (googleConnected) {
      showToast(
        "Dein Google-Konto ist über den Google Kalender mit Human Holo verbunden."
      );
      return;
    }

    const authWindow = window.open(
      "https://pam-holo-edge-guard.pamela-nitschke75.workers.dev/auth/google",
      "_blank",
      "noopener"
    );

    if (!authWindow) {
      window.location.href = "https://pam-holo-edge-guard.pamela-nitschke75.workers.dev/auth/google";
    }
  });
